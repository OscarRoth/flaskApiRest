...
async def crear_alumno(db: AsyncSession, alumno:
schemas.AlumnoCreateRequest):
nuevo_alumno = models.Alumno(**alumno.dict())
db.add(nuevo_alumno)
await db.commit()
await db.refresh(nuevo_alumno)
result = await db.execute(
select(models.Alumno).options(selectinload(models.Alumno.usuario)).w
here(models.Alumno.id == nuevo_alumno.id)
)
